 ...
 */
bool verify_domain_name(const uint64_t *chain_id, const uint8_t *addr) {
    bool ret = false;
 ...
 */
static uint32_t get_uint_from_data(const s_tlv_data *data, uint32_t *value) {
    uint8_t size_diff;
...
static bool parse_tlv(const s_tlv_payload *payload,
                      s_domain_name_info *domain_name_info,
                      s_sig_ctx *sig_ctx) {
    s_tlv_handler handlers[] = {{.tag = STRUCTURE_TYPE, .func = &handle_structure_type},
                                ...
                                {.tag = DOMAIN_NAME, .func = &handle_domain_name},
...
void handle_provide_domain_name(uint8_t p1, uint8_t p2, const uint8_t *data, uint8_t length) {
    ...
    if (g_tlv_payload.size == g_tlv_payload.expected_size) {
        g_domain_name_info.name = domain_name;
